...
class Zhubajie(object):
    ...
    def get_page_href(self):
        '''获取每一页的url'''
        url_kw = urllib.quote(self.kw)  # url编码
        page_1 = ''
        href_list = []
        href_list.append(self.start_url.format(page_1, url_kw))
        kw = 'k{}.html'
        for i in range(2,101):
            page =kw.format((i-2)*40+34)
            href_list.append(self.start_url.format(page, url_kw))
        return href_list


    def get_page_url(self, proxies, href):
        '''
        获取每一页上所有公司的url
        return ：list列表，存放此页所有公司的url
        '''
        response = requests.get(href, headers=headers, timeout=5, proxies=proxies, allow_redirects=False)
    ...
    def extractor_html(self, proxies, com_url):
        '''
        解析公司的url
        ...
        response = requests.get(com_url, headers=headers, timeout=5, proxies=proxies, allow_redirects=False)
        ...
        try:
            ...
            # print(
            # com_title.strip().encode('utf8'), com_name.strip().encode('utf8'), com_city.strip().encode('utf8'),
            # labels.strip().encode('utf8'), com_introduce.strip().encode('utf8'))
            # print(com_title)
            # print(com_name)
            # print(labels)
            # print(sec_city)
    ...
    def insert(self, com_title, com_name, com_city, labels, com_introduce):
        ...
        try:
            with connection.cursor() as c:
                # c.execute("insert into zhubajie_data(com_title, com_name, com_city, labels, com_introduce) values('{}', '{}', '{}', '{}', '{}')".format(com_title.strip(), com_name.strip(),com_city.strip(), labels.strip(),com_introduce.strip()))
                c.execute("insert into zhubajie_data_copy1(com_title, com_name, com_city, labels, com_introduce) values('{}', '{}', '{}', '{}', '{}')".format(com_title.strip().encode('utf8'), com_name.strip().encode('utf8'), com_city.strip().encode('utf8'), labels.strip().encode('utf8'), com_introduce.strip().encode('utf8')))
            connection.commit()
        except Exception as e:
    ...
    def ip_proxy(self):
        '''更换ip'''
        with codecs.open('ip.txt', 'r', encoding='utf-8') as r:
            ipmes = r.readlines()
        ip = ipmes[int(random.random() * len(ipmes))]
        print('ip addr:', ip)
        proxies = {'http': ip}
        return proxies

    def run(self):
        href_list = self.get_page_href()
        for href in href_list:
            print(href)
            proxies = self.ip_proxy()
            url_list = self.get_page_url(proxies, href)
            for url in url_list:
                print(url)
                proxies = self.ip_proxy()
                self.extractor_html(proxies, url)
                time.sleep(random.randint(1, 3))

        # self.extractor_html(com_url= 'https://shop.zbj.com/5493242/')
